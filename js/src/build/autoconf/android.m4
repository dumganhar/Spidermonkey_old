...
if test "$OS_TARGET" = "Android" -a -z "$gonkdir"; then
    echo "CPU_ARCH:$CPU_ARCH, MOZ_ARCH:$MOZ_ARCH"
    ...
    if test -z "$STLPORT_CPPFLAGS$STLPORT_LDFLAGS$STLPORT_LIBS"; then
        if test -n "$MOZ_ANDROID_LIBSTDCXX" ; then
            echo "==> $android_ndk/sources/cxx-stl/gnu-libstdc++/$android_gnu_compiler_version/libs/$ANDROID_CPU_ARCH/libgnustl_static.a"
            echo "==> $android_ndk/sources/cxx-stl/gnu-libstdc++/libs/$ANDROID_CPU_ARCH/libgnustl_static.a"
            echo "==> $android_ndk/sources/cxx-stl/gnu-libstdc++/libs/$ANDROID_CPU_ARCH/libstdc++.a"
            echo "==> ANDROID_CPU_ARCH:$ANDROID_CPU_ARCH"
